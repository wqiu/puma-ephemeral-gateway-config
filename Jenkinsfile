pipeline {
    ...
    stages {
        ...
        stage('Testing Docker image') {
            steps {
                script {
                    env.GATEWAY_CONTAINER_ID = sh script: "docker run --env ACCEPT_LICENSE=true --env EXTRA_JAVA_ARGS=-Dcom.l7tech.bootstrap.env.license.enable=true --env -p 8080 -p 8443 -d ${env.NEW_IMAGE_NAME}:${env.NEW_IMAGE_TAG} | cut -c 1-12", returnStdout: true
